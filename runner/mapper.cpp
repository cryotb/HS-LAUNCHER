...
std::uint16_t mapper::load_library(void* file_buffer, HANDLE process_handle, uint8_t* loader_bytes, size_t loader_size)
{
	auto* const dos_header = static_cast<PIMAGE_DOS_HEADER>(file_buffer);

	if (dos_header == nullptr)
	{
		LOG_INFO("load_library() ->  invalid image headers have been found.");
		return 0xE674;
	}
	...
	auto thread_id = GetThreadId(h_thread);
	auto thread_ctx = CONTEXT{};

	if (!GetThreadContext(h_thread, &thread_ctx))
		return 0xE44;

	LOG_INFO("thread_id =  {}", thread_id);
	LOG_INFO("thread_ip =  {:x}", thread_ctx.Rip);
	...
	if (!loader_job_info.DidComplete)
	{
		...
		switch (loader_job_info.ErrorCode)
		{
		case 0xE02:
			strcpy_s(reason, _XS("missing dependency"));
			break;
		default:
			strcpy_s(reason, _XS("N/A"));
			break;
		}

		LOG_INFO("loader job has failed with code {:x} and reason {}.",
			loader_job_info.ErrorCode, reason);
